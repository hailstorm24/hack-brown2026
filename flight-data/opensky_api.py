...
class OpenSkyApi(object):
    ...
    def __init__(self, username=None, password=None, token=None):
        """Create an instance of the API client. If you do not provide username/password or token,
        requests will be anonymous which imposes some limitations.

        :param str username: an OpenSky username (optional).
        :param str password: an OpenSky password for the given username (optional).
        :param str token: OAuth2 access token (optional). Use when authenticating via client credentials.
        """
    ...
    def _get_json(self, url_post, callee, params=None):
        ...
        if r.status_code == 200:
            ...
            try:
                self._last_response_body = r.text
            except Exception:
    ...
    def _check_rate_limit(self, time_diff_noauth, time_diff_auth, func):
        ...
        :rtype: bool
        """
        if len(self._auth) >= 2 or self._token:
    ...
    def get_flights_by_aircraft(self, icao24, begin, end):
        ...
        if states_json is not None:
            return [FlightData(list(entry.values())) for entry in states_json]
        # 404 = no flights in period (API returns 404 with empty body)
        if getattr(self, "_last_status_code", None) == 404:
            return []
        return None
