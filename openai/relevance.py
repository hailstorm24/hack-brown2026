...
Loads API key from OPENAI_API_KEY (env var or optional .env via python-dotenv).
"""
...
try:
    ...
    OpenAI = None  # pip install openai

# Seconds to wait for one API call before giving up (avoids stalling)
_REQUEST_TIMEOUT = 60

# Load .env from this directory if python-dotenv is installed (optional)
try:
...
_PROMPT = (
    ...
    "(why would a CEO's private plane visit this location: vacation / headquarters / which known partners / unknown relevance?)"
)
